...
`ADMIN_PASSWORD = process.env.ADMIN_PASSWORD ?? "scoop2026"` — a fallback secret committed in the clear. The session cookie stored that same value, with no `secure` flag, and the check was `session === ADMIN_PASSWORD` against a repo-visible literal. Trivially forgeable with curl.
